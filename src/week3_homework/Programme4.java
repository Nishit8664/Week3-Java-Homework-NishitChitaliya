...
public class Programme4 {
    public static void main(String[] args) {
        ...
        isLeapYear(year);               //calling isLeapYear method
        getDaysInMonth(month, year);    //calling getDaysInMonth method
        scanner.close();
    }

    public static boolean isLeapYear(int year) {
        if (year >= 1 && year <= 9999) {                        //If inside if to find leap year
            if (year % 4 == 0) {
    ...
    public static int getDaysInMonth(int month, int year) {
        if (month < 1 || month > 12) {                              //If to find valid month
            throw new IllegalArgumentException("Month is not in range 1-12");  //If not valid month it will throw the error
        } else {
            switch (month) {                                         //Switch to find days in month
                case 1:
                case 3:
                case 5:
                case 7:
                ...
                case 12:
                    ...
                    break;
                case 2:
                    if (isLeapYear(year)) {                         //if inside switch to find leap year or not
